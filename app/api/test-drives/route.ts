...
export async function POST(request: NextRequest) {
  ...
  if (!user) return Response.json({ error: "Please login to request a test drive" }, { status: 401 });

  const { vehicle_id, preferred_date, preferred_time, name } = await request.json();
